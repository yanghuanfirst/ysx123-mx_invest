...
class RecipeController extends BaseController
{
    ...
    function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            ...
            if (empty($userInfo) && in_array($action->id, $this->requireLoginActions)) {
                echo json_encode(Util::jsonReturn(ResponseCode::LOGIN_REQUIRED,'no login'));exit();
                return false;
            }
            return true;
        }
    ...
    function actionRecipeType():array
    {
        ...
        return Util::jsonReturn(0, 'success', ["type_list"=>$result]);
    }
    /**
    ...
    function actionIndex():array
    {
        ...
        if (!$recipeModel->validate()) {
            return Util::jsonReturn(ResponseCode::PARAM_CHECK_FAIL, current($recipeModel->getFirstErrors()));
        }
        $map = ["and"];
        if($title){
    ...
    function actionUploadImage(){
        ...
        if (!$model->validate()) {
            ...
            return Util::jsonReturn(ResponseCode::PARAM_CHECK_FAIL, current($model->getFirstErrors()));
        }
        ...
        return Util::jsonReturn(0, 'success',["url"=>$backImage]);
    }
    ...
    function actionAddRecipe():array
    {
        ...
        if (!$res){
            return Util::jsonReturn(-1, "add recipe fail please try again");
        }
        return Util::jsonReturn(0, 'success'); //新增成功
    }
    ...
    function actionDelRecipe():array
    {
        ...
        if (!$info){
            return Util::jsonReturn(-1, "recipe not exist");
        }
        $res = $info->delete();
        if (!$res){
            return Util::jsonReturn(-1, "delete recipe fail please try again");
        }
        return Util::jsonReturn(0, 'success'); //删除成功
    }
    ...
    function actionCollectList():array
    {
        ...
        $total = 0;
        if (empty($recipeIds)){
            return Util::jsonReturn(0, 'success',compact("list","total"));
        }
        $offset = ($page - 1) * $pageSize;
        $map = ["and"];
        if($title){
    ...
    function actionCollect():array
    {
        ...
        try{
            ...
            return Util::jsonReturn(-1, "action fail please try again");
        }
        return Util::jsonReturn(0, 'success');
    }
    ...
    function actionDetail():array
    {
        ...
        return Util::jsonReturn(0, 'success',compact("info"));
    }
    ...
    function actionEditRecipe():array
    {
        ...
        $res = $recipeModel->save();
        if (!$res){
            return Util::jsonReturn(-1, "edit recipe fail please try again");
        }
        return Util::jsonReturn(0, 'action success');
    }
    ...
    function actionMyRecipe():array
    {
        ...
        return Util::jsonReturn(0, 'success', compact('total','list',"collectCount"));
    }
    ...
    function actionAddComment():array
    {
        ...
        $comment->save();
        return Util::jsonReturn(0, 'Review success');
    }
    ...
    function actionCommentList():array
    {
        ...
        return Util::jsonReturn(0, 'success',compact("total","list"));
    }
    ...
    function actionDeleteComment():array
    {
        ...
        if (!$info){
            return Util::jsonReturn(-1, "comment not exist");
        }
        $info->delete();
        return Util::jsonReturn(0, 'delete success');
    }
